...
export default function ChangePasswordPage() {
  ...
  const validateCurrentPassword = (password: string) => {
    if (!password) {
      setCurrentPasswordError(
        t("change_password_page.validation.required_current"),
      );
      return false;
    }
    setCurrentPasswordError("");
  ...
  const validateNewPassword = (password: string) => {
    ...
    if (!/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(password)) {
      setNewPasswordError(
        t("change_password_page.validation.password_requirements"),
      );
      return false;
    }
  ...
  const validateConfirmPassword = (confirm: string) => {
    if (!confirm) {
      setConfirmPasswordError(
        t("change_password_page.validation.required_confirm"),
      );
      return false;
    }
    ...
    if (confirm !== newPassword) {
      setConfirmPasswordError(
        t("change_password_page.validation.password_mismatch"),
      );
      return false;
    }
      ...
      <div className="max-w-[420px] w-full space-y-8 relative z-10">
        <div className="backdrop-blur-sm rounded-2xl shadow-2xl p-6 sm:p-8 border auth-card">
          <div className="text-center mb-6">
            ...
            <p className="auth-text">{t("change_password_page.subtitle")}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6" noValidate>
            <div>
              ...
              <input
                ...
                placeholder={t(
                  "change_password_page.current_password_placeholder",
                )}
            ...
            <div>
              ...
              {newPasswordTouched && !newPasswordError && newPassword && (
                <p className="mt-1 text-sm" style={{ color: "#22c55e" }}>
                  {t("change_password_page.password_strong")}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder={t(
                  "change_password_page.confirm_password_placeholder",
                )}
                className="w-full px-4 py-3 border-2 rounded-lg outline-none transition-all disabled:cursor-not-allowed disabled:opacity-60 auth-input"
                style={{
                  borderColor:
              ...
              {confirmPasswordTouched &&
                ...
                confirmNewPassword && (
                  <p className="mt-1 text-sm" style={{ color: "#22c55e" }}>
                    {t("change_password_page.passwords_match")}
                  </p>
                )}
            </div>
            ...
            <LoginButton
              loading={loading}
              text={t("change_password_page.change_btn")}
            />
