...
export const TableMap2D: React.FC<TableMap2DProps> = ({
    ...
    const filteredTables = React.useMemo(() => {
        return tables.filter(table => {
            if (filter?.floor && filter.floor !== 'all' && table.area !== filter.floor) return false;
            return true;
        });
    }, [tables, filter]);

    const handleTableDragEnd = (tableId: string, newPosition: { x: number; y: number }) => {
        ...
        for (const targetTable of tables) {
            ...
            const dx = (newPosition.x + TABLE_SIZE / 2) - (targetTable.position.x + TABLE_SIZE / 2);
            const dy = (newPosition.y + TABLE_SIZE / 2) - (targetTable.position.y + TABLE_SIZE / 2);
    ...
    const handleZoneDragStart = (e: React.DragEvent, index: number, zoneId: string) => {
            ...
            >
                ...
                {zones.map((zone, index) => (
                    <div
                        ...
                        onDrop={(e) => { e.stopPropagation(); handleZoneDrop(e, index); }}
                        style={{
                            position: 'absolute',
                            top: zone.y,
                            left: 0,
                            ...
                            backgroundColor: draggingZoneId === zone.id ? 'rgba(0,0,0,0.05)' : 'rgba(255,255,255,0.02)', // Subtle fill
                            zIndex: 1,
                            transition: 'all 0.2s',
                            userSelect: 'none',
                        }}
                            ...
                            <div style={{ flex: 1 }}>
                                {editingZoneId === zone.id ? (
                                    <input
                                        ...
                                        onDragStart={(e) => { e.preventDefault(); e.stopPropagation(); }} // Prevent drag when editing
                                        style={{
                                            fontSize: 16,
                                            fontWeight: 'bold',
                                            color: 'var(--text)',
                                    ...
                                    <div
                                        onDoubleClick={(e) => { e.stopPropagation(); startEditing(zone); }}
                                        title={readOnly ? "" : "Double click to rename, Drag header to reorder"}
                                        style={{
                                            fontSize: 16,
                                            fontWeight: 'bold',
                                            color: 'var(--text)',
                                    ...
                                    >
                                        ...
                                        {!readOnly && onZoneDelete && (
                                            <button
                                                ...
                                                onMouseEnter={(e) => e.currentTarget.style.color = '#FF380B'}
