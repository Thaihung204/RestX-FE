...
const getSteps = (t: (key: string) => string) => [
  {
    ...
    script: [
      ...
      },
      {
        ...
        heading: t('tour.reservations.steps.step_1.script.select_table.heading'),
    ...
    script: [
      {
        ...
        heading: t('tour.reservations.steps.step_2.script.receive_request.heading'),
        body: t('tour.reservations.steps.step_2.script.receive_request.body'),
      },
      {
        icon: <CheckCircleOutlined />,
        heading: t('tour.reservations.steps.step_2.script.confirm_or_adjust.heading'),
        body: t('tour.reservations.steps.step_2.script.confirm_or_adjust.body'),
      },
      {
        icon: <TeamOutlined />,
        heading: t('tour.reservations.steps.step_2.script.assign_staff.heading'),
  ...
  },
  {
    ...
    script: [
      ...
      {
        icon: <EnvironmentOutlined />,
        heading: t('tour.reservations.steps.step_3.script.guide_to_table.heading'),
        body: t('tour.reservations.steps.step_3.script.guide_to_table.body'),
      },
      {
        icon: <CompassOutlined />,
        heading: t('tour.reservations.steps.step_3.script.update_table_status.heading'),
        body: t('tour.reservations.steps.step_3.script.update_table_status.body'),
      },
    ],
  },
];
...
/*Script item*/
function ScriptItem({
  icon, heading, body, index,
}: {
  icon: React.ReactNode; heading: string; body: string; index: number;
}) {
  return (
    <motion.div
      ...
      style={{
        ...
        borderRadius: 14, background: 'var(--bg-base)',
        border: '1px solid var(--border)',
      }}
    >
      <div style={{
        width: 36, height: 36, borderRadius: 10, flexShrink: 0,
        background: CL, border: `1px solid ${CB}`,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        color: COLOR, fontSize: 15, marginTop: 2,
      }}>
        {icon}
      </div>
      <div>
        <Text style={{ fontWeight: 700, color: 'var(--text)', fontSize: 14, display: 'block', marginBottom: 4 }}>
          {heading}
        </Text>
...
/*Step block*/
function StepBlock({
  step, isOpen, onClick, stepIndex,
}: {
  step: ReturnType<typeof getSteps>[0]; isOpen: boolean; onClick: () => void; stepIndex: number;
}) {
  return (
    <motion.div
      ...
      transition={{ duration: 0.6, delay: stepIndex * 0.05, ease: [0.25, 0.4, 0.25, 1] }}
    ...
    >
      <div style={{
        ...
        boxShadow: isOpen ? `0 8px 40px rgba(245,158,11,0.12)` : '0 2px 12px rgba(0,0,0,0.06)',
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <Text style={{
              fontSize: isOpen ? 15 : 16, fontWeight: 700,
              color: 'var(--text)', display: 'block', lineHeight: 1.3,
            }}>
              {step.title}
            </Text>
            {!isOpen && (
              <Text style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4, display: 'block' }}>
                {step.summary}
              </Text>
            )}
          </div>
...
export default function ReservationsTourPage() {
  ...
  const visibleSteps = steps.filter((s) =>
    s.id === 1 || openSteps.includes(s.id - 1)
  );
  ...
  return (
    <div style={{ overflow: 'hidden' }}>
      <section style={{
        ...
        <div ref={containerRef} style={{ maxWidth: 900, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          >
            ...
            <Title level={1} style={{
              ...
              margin: '0 0 16px', color: 'var(--text)', lineHeight: 1.1,
            }}>
            ...
            <Paragraph style={{ fontSize: 16, color: 'var(--text-muted)', margin: 0, maxWidth: 480, marginInline: 'auto' }}>
                ...
                >
                  ...
                  {idx > 0 && (
                    <motion.div
                      ...
                      style={{
                        width: 2, background: `linear-gradient(to bottom, ${CB}, transparent)`,
                        margin: '-8px auto 0', marginBottom: 8,
                      }}
                    />
                  )}
                  <StepBlock
